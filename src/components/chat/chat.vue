...
let tips = reactive([
   { tipId: 1, title: '回旋是怎么练成的', username: '杰洛.齐贝林', avatarUrl: 'https://avatars.githubusercontent.com/u/94109480?v=4', description: '对于回旋是怎么练成的这个话题,乔尼有话说...', tipPicture: '/src/assets/image/j5.jpg' },
...
function getAllSimpleTip() {//获取帖子列表
   ...
   axios.get(baseURL + '/getAllSimpleTip').then(
      response => {
         ...
         tips.push({ tipId: 1, title: '回旋是怎么练成的', description: '对于回旋是怎么练成的这个话题,乔尼有话说...', tipPicture: '/src/assets/image/j5.jpg' })
         tips.push({ tipId: 2, title: 'chatgpt现在允许免费用户每天最多生成两张由dall-e-3制作的图片', description: 'OpenAI 表示,使用《DALL-E 3》创建图像的功能正在"推出",但您可能已经可以使用了:在撰写本文时,在尝试使用 ChatGPT Mac 应用程序创建了两张图像后收到通知,说已经达到了当天的图像创建上限', tipPicture: 'https://img-s-msn-com.akamaized.net/tenant/amp/entityid/AA1ouckW.img?w=673&h=1119&m=6' })
         tips.push({ tipId: 3, title: 'Madao观察日记', description: '暑假做了关于Madao的观察日记,MADAO是公园的主人,长着胡子戴着墨镜的生物,基本一整天都不干活在公园里呆着...', tipPicture: '/src/assets/image/803.jpg' })
      })
}
